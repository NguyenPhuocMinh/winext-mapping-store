...
function MappingStore(params = {}) {
  ...
  this.mapping = async function (app, router) {
    ...
    loggerTracer.info(chalk.green(`Load function mapping by ${name}-${version} successfully!`));
    /**
      ...
      if (!isEmpty(mappings) && isArray(mappings)) {
        ...
        return mappings.map(mapping => {
          ...
          app.use(contextPath, router[toLower(method)](pathName, (
            request, response, next) => handleMapping({ request, response, input, output, service, requestId, loggerFactory, loggerTracer })
          ));
