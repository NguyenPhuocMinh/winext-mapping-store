      ...
      if (!isEmpty(mappings) && isArray(mappings)) {
        ...
        return mappings.map((mapping) => {
          ...
          if (isEmpty(authorization)) {
            loggerFactory.error(`Authorization not found`, {
              requestId: `${requestId}`,
            });
          ...
          app.use(
            ...
            router[toLower(method)](pathName, (request, response, next) =>
              authorization.noVerifyToken(request, response, next)
            ),
            router[toLower(method)](pathName, (request, response, next) =>
              authorization.verifyTokenMiddleware(request, response, next)
            ),
            router[toLower(method)](pathName, (request, response, next) =>
              authorization.publicRouters(request, response, next)
            ),
            router[toLower(method)](pathName, (request, response, next) =>
              authorization.protectedRouters(request, response, next)
            ),
            router[toLower(method)](pathName, (request, response, next) =>
              handleCaching({
                request,
