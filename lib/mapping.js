...
function MappingStore(params = {}) {
  ...
  this.mapping = async function (app, router) {
    loggerFactory.warn(`Mapping has been start`, {
      requestId: `${requestId}`,
    });

    loggerTracer.warn(chalk.yellow(`Load Func mapping by ${name}-${version} successfully!`));
    /**
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!isEmpty(mappings) && isArray(mappings)) {
        for (let i = 0; i < mappings.length; i++) {
          ...
          if (Object.prototype.hasOwnProperty.call(service.register, 'reference')) {
             ...
             */
            if (Object.prototype.hasOwnProperty.call(reference, 'dataStore')) {
              const _dataStore = reference.dataStore;
              if (_dataStore === 'app-repo-store/dataStore') {
                serviceParams.dataStore = repoStore.dataStore;
              }
            }
            /**
             * Add dataSequelize to params service
             */
            if (Object.prototype.hasOwnProperty.call(reference, 'dataSequelize')) {
              const _dataSequelize = reference.dataSequelize;
              if (_dataSequelize === 'app-repo-store/dataSequelize') {
                serviceParams.dataSequelize = repoStore.dataSequelize;
              }
            }
            /**
      ...
      .then((info) => {
        loggerFactory.warn(`Router mapping has complete`, {
          requestId: `${requestId}`,
        });
